    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.projects.update", [$project->id]) }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text" name="slug" id="slug" value="{{ old('slug', $project->slug) }}" required>
                @if($errors->has('slug'))
                    <div class="invalid-feedback">
                        {{ $errors->first('slug') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('folder') ? 'is-invalid' : '' }}" type="text" name="folder" id="folder" value="{{ old('folder', $project->folder) }}" required>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('allowed_users') ? 'is-invalid' : '' }}" name="allowed_users[]" id="allowed_users" multiple>
                    @foreach($allowed_users as $id => $allowed_user)
                        <option value="{{ $id }}" {{ (in_array($id, old('allowed_users', [])) || $project->allowed_users->contains($id)) ? 'selected' : '' }}>{{ $allowed_user }}</option>
                    @endforeach
                </select>
