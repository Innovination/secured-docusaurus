            ...
            <form method="POST" action="{{ route('admin.projects.update', [$project->id]) }}" enctype="multipart/form-data">
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('project_name') ? 'is-invalid' : '' }}" type="text"
                        name="project_name" id="project_name" value="{{ old('project_name', $project->project_name) }}"
                        required>
                ...
                <div class="form-group">
                    ...
                    <input class="form-control {{ $errors->has('slug') ? 'is-invalid' : '' }}" type="text"
                        name="slug" id="slug" value="{{ old('slug', $project->slug) }}" required>
                    @if ($errors->has('slug'))
                        <div class="invalid-feedback">
                            {{ $errors->first('slug') }}
                        </div>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control {{ $errors->has('status') ? 'is-invalid' : '' }}" name="status" id="status" required>
                        <option value="" disabled {{ old('status', $project->status) === null ? 'selected' : '' }}>{{ trans('global.pleaseSelect') }}</option>
                ...
                <div class="form-group">
                    ...
                    <textarea class="form-control {{ $errors->has('remarks') ? 'is-invalid' : '' }}" name="remarks" id="remarks">{{ old('remarks', $project->remarks) }}</textarea>
                ...
                <div class="form-group">
                    ...
                    <select class="form-control select2 {{ $errors->has('allowed_users') ? 'is-invalid' : '' }}" name="allowed_users[]" id="allowed_users" multiple>
                        @foreach ($allowed_users as $id => $allowed_user)
                            <option value="{{ $id }}" {{ in_array($id, old('allowed_users', [])) || $project->allowed_users->contains($id) ? 'selected' : '' }}>
                                {{ $allowed_user }}</option>
                        @endforeach
                    </select>
